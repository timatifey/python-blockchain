...
class Block:
    ...
    def __init__(self, index, prev_hash, node_id, nonce_type=None, data=None, hash=None, nonce=None):
        ...
        self.data = data or self.generate_random_string(256)
        self.hash = hash or self.generate_hash(nonce_type)
    ...
    def update_nonce(self, nonce_type):
        ...
        self.nonce += nonce_additional.get(nonce_type) or default_nonce_additional
    ...
    def __str__(self):
        return f'Generated by Node[{self.node_id}]:\n' + \
            f'\tIndex = {self.index},\n' + \
            f'\tHash = {self.hash},\n' + \
            f'\tPrevious hash = {self.prev_hash},\n' + \
            f'\tData = {self.data[:len(self.hash)]}...,\n' + \
            f'\tNonce = {self.nonce}\n' + \
            '-' * 20
...
def build_new_block(index, prev_hash, node_id, nonce_type) -> Block:
    """Build new block."""
    return Block(index, prev_hash, node_id, nonce_type)


def build_block_from_json(block_json) -> Block:
    """Build block from json."""
    block = json.loads(block_json, object_hook=lambda d: SimpleNamespace(**d))
    return Block(
...
GENESIS_INDEX = 0
GENESIS_BLOCK = build_new_block(index=GENESIS_INDEX, prev_hash='GENESIS', nonce_type=1, node_id=-1)
